...
// array_push($where, 'ORDER BY duedate DESC');
...
if($order[0]['column'] != 7 && $order[0]['dir'] != 'asc') {
    usort($rResult, "sortFunction");
}
...
foreach ($rResult as $aRow) {
    ...
    if (is_numeric($clientid) || $project_id) {
        $numberOutput = '<a href="' . admin_url('bills/list_invoices/' . $aRow['id']) . '" target="_blank">' . format_invoice_number($aRow['id']) . '</a>';
    } else {
        $numberOutput = '<a href="' . admin_url('bills/list_invoices/' . $aRow['id']) . '" onclick="init_bill(' . $aRow['id'] . '); return false;">' . format_invoice_number($aRow['id']) . '</a>';
    }
    ...
    if (has_permission('invoices', '', 'edit')) {
        $numberOutput .= ' | <a href="' . admin_url('bills/invoice/' . $aRow['id']) . '">' . _l('edit') . '</a>';
    }
    $numberOutput .= '</div>';
