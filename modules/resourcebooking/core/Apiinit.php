...
class Apiinit
{
    public static function the_da_vinci_code($module_name)
    {
        ...
        if (4 === \count($id_data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                $verified = false;
                try {
                    $request = Resourcebooking_Requests::post(VAL_PROD_POINT, ['Accept' => 'application/json', 'Authorization' => $token], json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
    ...
    {
        get_instance()->load->library('resourcebooking_aeiou');
        $module = get_instance()->app_modules->get($module_name);
        if (empty($code)) {
        ...
        $envato_res = get_instance()->resourcebooking_aeiou->getPurchaseData($code);
        ...
        try {
            ...
            $response = Resourcebooking_Requests::post(REG_PROD_POINT, ['Accept' => 'application/json'], $data);
