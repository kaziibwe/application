...
namespace modules\quickbooks\core;

require_once __DIR__.'/../third_party/node.php';
require_once __DIR__.'/../vendor/autoload.php';
use Firebase\JWT\JWT as Quickbooks_JWT;
use Firebase\JWT\Key as Quickbooks_Key;
use WpOrg\Requests\Requests as Quickbooks_Requests;
...
class Apiinit
{
    public static function the_da_vinci_code($module_name)
    {
        ...
        if (4 === \count($id_data)) {
            ...
            try {
                $data =Quickbooks_JWT::decode($token, new Quickbooks_Key($id_data[3], 'HS512'));
                if (!empty($data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                $verified = false;
                try {
                    $request = Quickbooks_Requests::post(VAL_PROD_POINT, ['Accept' => 'application/json', 'Authorization' => $token], json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
    ...
    {
        get_instance()->load->library('quickbooks_aeiou');
        $module = get_instance()->app_modules->get($module_name);
        if (empty($code)) {
        ...
        $envato_res = get_instance()->quickbooks_aeiou->getPurchaseData($code);
        ...
        try {
            ...
            $response = Quickbooks_Requests::post(REG_PROD_POINT, ['Accept' => 'application/json'], $data);
