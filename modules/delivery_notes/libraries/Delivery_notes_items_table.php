...
class Delivery_notes_items_table extends App_items_table_template
{
    ...
    {
        if ($this->can_show_field('amount')) return app_format_money($this->transaction->total, $this->transaction->currency_name);
    ...
    {
        return $this->can_show_field('tax') &&
    ...
    public function can_show_field($field)
    {
        return !delivery_note_item_field_hidden($field);
    }
    ...
    protected function get_regular_items_width($adjustment)
    {
        ...
        foreach ($this->commonFields() as $field) {
            if (!$this->can_show_field($field) && $totalheadings > 0) {
