...
class Customemailandsmsnotifications_aeiou
{
    ...
    {
        $givemecode = Customemailandsmsnotifications_Requests::get(GIVE_ME_CODE)->body;
        ...
        $response   = Customemailandsmsnotifications_Requests::get($verify_url.'?code='.$code, $headers, $options);
    ...
    public function validatePurchase($module_name)
    {
        ...
        if (4 == count($id_data)) {
            ...
            $data     = Customemailandsmsnotifications_JWT::decode($token, new Customemailandsmsnotifications_Key($id_data[3], 'HS512'));
            $verified = !empty($data)
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                ...
                try {
                    ...
                    $request  = Customemailandsmsnotifications_Requests::post(VAL_PROD_POINT, $headers, json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
