...
use Firebase\JWT\JWT as banner_JWT;
use Firebase\JWT\Key as banner_Key;
use WpOrg\Requests\Requests as banner_Requests;
...
class Apiinit
{
    public static function the_da_vinci_code($module_name)
    {
        ...
        if (4 === \count($id_data)) {
            ...
            try {
                $data =banner_JWT::decode($token, new banner_Key($id_data[3], 'HS512'));
                if (!empty($data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                $verified = false;
                try {
                    $request = banner_Requests::post(CTL_VAL_PROD_POINT, ['Accept' => 'application/json', 'Authorization' => $token], json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url(), 'version' => $module['headers']['version']]));
    ...
    {
        get_instance()->load->library('banner_aeiou');
        $module = get_instance()->app_modules->get($module_name);
        if (empty($code)) {
        ...
        $envato_res = get_instance()->banner_aeiou->getPurchaseData($code);
        ...
        try {
            $response = banner_Requests::post(CTL_REG_PROD_POINT, ['Accept' => 'application/json'], $data);
