...
class Whatsbot_aeiou
{
    ...
    {
        $givemecode = whatsbot_Requests::get(CTL_GIVE_ME_CODE)->body;
        ...
        if (4 == count($id_data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                ...
                try {
                    ...
                    $request  = whatsbot_Requests::post(CTL_VAL_PROD_POINT, $headers, json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url(), 'version' => $module['headers']['version']]));
