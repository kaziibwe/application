...
use Firebase\JWT\JWT as whatsbot_JWT;
use Firebase\JWT\Key as whatsbot_Key;
...
class Apiinit
{
    public static function the_da_vinci_code($module_name)
    {
        ...
        if (4 === \count($id_data)) {
            ...
            try {
                $data =whatsbot_JWT::decode($token, new whatsbot_Key($id_data[3], 'HS512'));
                if (!empty($data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                $verified = false;
                try {
                    $request = whatsbot_Requests::post(CTL_VAL_PROD_POINT, ['Accept' => 'application/json', 'Authorization' => $token], json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url(), 'version' => $module['headers']['version']]));
    ...
    public static function pre_validate($module_name, $code = '')
    {
        ...
        try {
            $response = whatsbot_Requests::post(CTL_REG_PROD_POINT, ['Accept' => 'application/json'], $data);
